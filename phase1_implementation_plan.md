...
> **Current state:** Phase 0 and Phase 1 Goals 1–6 complete. All entities built. TenantRepository, UserRepository, TenantUserRepository, ProjectRepository, and SoftDeleteService created. Ready for Goal 7 (RLS isolation integration test).
>
...
### What to build
- [x] Add `@Where(clause = "deleted_at IS NULL")` to every entity that has `deleted_at`
- [x] Create `TenantRepository extends JpaRepository<Tenant, UUID>`
- [x] Create `UserRepository extends JpaRepository<User, UUID>`
- [x] Create `TenantUserRepository extends JpaRepository<TenantUser, UUID>` *(added — needed for Phase 2 auth)*
- [x] Create `ProjectRepository extends JpaRepository<Project, UUID>`
- [x] Create a `SoftDeleteService` helper with `softDelete(entity, repo)` and `restore(entity, repo)` methods
